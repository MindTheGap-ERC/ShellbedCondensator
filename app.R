...
ui <- navbarPage(title=tags$b("The Shellbed Condensator"),
                 ...
                 tabPanel(title ="Introduction",
                          ...
                          tags$p("Fossil concentrations are landmarks that are easily recognizable in the rock face (Figs. 1,3). They can indicate important biological and ecological events such as mass mortality, or record times of high productivity.
                                 However, non-biological processes such as gradual sorting by currents or violent accumulation by storms are also known to generate fossil concentrations. Thus, a profound understanding of deposition is required to reconstruct the original biological processes from its sedimentary record.
                                 "),
                          tags$h4("The Question"),
                          tags$p(HTML("By varying both sedimentation rate and fossil input, Prof. Susan Kidwell (University of Chicago) showed in a seminal paper Science paper <a href=https://doi.org/10.1038/318457a0 target=\"_blank\"> (Kidwell 1985) </a> that patterns of fossil abundance in a section can be produced in different ways.
                                 Fossil concentrations such as bone or shell beds can form as a result of high input of organism remains into the sediment or of reduced sediment input, as the latter reduces the the amount of sediment placed between individual fossils and generates a sedimentary condensation.
                          ...
                          actionButton(inputId = "go_to_app_2",
                          ...
                          If you would like to apply the DAIME model to your own data, you can find an example with annotated R code in the article by <a href=https://doi.org/10.1029/2020PA003979 target=\"_blank\" > Jarochowska et al. (2020) </a>.
                          The formation of fossil concentrations in this App has been simplified to illustrate the main concepts, but in reality there are further factors affecting their formation, including bioturbation and burial <a href=https://doi.org/10.1017/pab.2015.30 target=\"_blank\"> (Tomašových 
                          et al., 2016) </a>, as well as cementation and dissolution <a href=https://doi.org/10.2110/palo.2008.p08-033r target=\"_blank\" > (Tomasovych &  Schlögl, 2008) </a>")),
                          ...
                          tags$p(HTML("The App is based on the DAIME model by Hohmann (<a href=https://doi.org/10.13140/RG.2.2.23372.51841 target=\"_blank\"> 2018</a>, <a href=https://doi.org/10.2110/palo.2020.038 target=\"_blank\" > 2021</a>), which is available as a package for the R Software <a href=https://cran.r-project.org/web/packages/DAIME/index.html target=\"_blank\" > (Hohmann 2020)</a>.
                                 ...
                                 Removing intervals with net erosion from this sediment accumulation history yields the age model. It consists of intervals that are either (1) eroded and do not preserve any information or (2) have positive sediment accumulation.
                                 For the latter, the age model provides a 1-1 correspondence between time of deposition and stratigraphic height.
                            ...
                            column(6,
                                   ...
                                   column(7,
                                        div(style="margin-left: -4em",  tags$ul("PhD candidate", br(),
                                                    ...
                                                    HTML("<a href=https://scholar.google.com/citations?hl=de&user=2CB_ktEAAAAJ target=\"_blank\" > Google Scholar profile </a>"))),
                                   )
                                   )
                            ),
                            column(6,
                          ...
                          tags$h4("How to Cite"),
                          tags$p("To cite this app, please use the following two references:"),
                            tags$ul(
...
"))
                 ...
                 #### AI: App Interface ####
                 tabPanel(title = "Visualizing Sedimentary Condensation and Erosion",
                          value="app",
    ...
    column( width=3,
            wellPanel(
              #### AI: Left Panel/Input Sliders ####
                      ...
                      checkboxInput("show_input_points",
                                    label="Show Points",
                                    value = TRUE),
                      fluidRow( column(4,
                                       tags$b("Time I1"),
                                ...
                                column(8,
                                       sliderInput(inputId = "y_pat_1",label='Value I1',value=0,min=0,max=1,step=0.05,animate = TRUE)
                                )),
                     fluidRow(
                              column(4,
                                     numericInput(inputId = "x_pat_2",label = "Time I2",value=0.25,min=0,max=1)),
                              column(8,
                                     sliderInput(inputId = "y_pat_2",label="Value I2",value=0.25,min=0,max=1,step=0.05,animate = TRUE)
                              )),
                     fluidRow(column(4,
                                     numericInput(inputId = "x_pat_3",label = 'Time I3',value=0.5,min=0,max=1)),
                              column(8,
                                     sliderInput(inputId = "y_pat_3",label='Value I3',value=0.5,min=0,max=1,step=0.05,animate = TRUE)
                              )),
                     fluidRow(column(4,
                                     numericInput(inputId = "x_pat_4",label = 'Time I4',value=0.75,min=0,max=1)),
                              column(8,
                                     sliderInput(inputId = "y_pat_4",label='Value I4',value=0.75,min=0,max=1,step=0.05,animate = TRUE)
                              )),
                     fluidRow(column(4,
                                     tags$b("Time I5"),
                                     tags$p(textOutput("end_of_observation_name_2",container = div))),
                              column(8,
                                     sliderInput(inputId = "y_pat_5",label='Value I5',value=1,min=0,max=1,step=0.05,animate = TRUE)
                            ) ),
                    ...
                    wellPanel(
                              ...
                              textInput("sed_content_name",
                                        "What is Placed in the Sediment?",
                                        value="Shell"),
                              textInput("sed_content_unit",
                                        "What are Its Units?",
                                        value="#"),
                              textInput("T_unit",
                                        "Time Unit",
                              ...
                              numericInput(inputId = "duration",
                                           ...
                                           step=0.001,
                                           min=0.001),
                              actionButton(inputId = 'update_model_options',
                                           label = "Apply Changes")
                    )
          )
    ),
    ...
    column(width=3,
           wellPanel(
             #### AI: Right Panel/Sedimentation Sliders ####
                     ...
                     checkboxInput("show_sed_points", label="Show Points", value = TRUE, width = NULL),
                     fluidRow(
                       column(4,
                              tags$b("Time S1"),
                              tags$p("0",style='padding: 20%;')),
                       column(8,
                              sliderInput(inputId = "y_sed_1",label='Value S1',value=1,min=-1,max=1,step=0.05,animate = TRUE))
                     ),
                     fluidRow(
                       column(4,
                              numericInput(inputId = "x_sed_2",label = 'Time S2',value=0.25,min=0,max=1)),
                       column(8,
                              sliderInput(inputId = "y_sed_2",label='Value S2',value=0.75,min=-1,max=1,step=0.05,animate=TRUE))
                     ),
                     fluidRow(
                       column(4,
                              numericInput(inputId = "x_sed_3",label = 'Time S3',value=0.5,min=0,max=1)),
                       column(8,
                              sliderInput(inputId = "y_sed_3",label='Value S3',value=0.5,min=-1,max=1,step=0.05,animate = TRUE))
                     ),
                     fluidRow(
                       column(4,
                              numericInput(inputId = "x_sed_4",label = 'Time S4',value=0.75,min=0,max=1)),
                       column(8,
                              sliderInput(inputId = "y_sed_4",label='Value S4',value=0.25,min=-1,max=1,step=0.05,animate = TRUE))
                     ),
                     fluidRow(
                       column(4,
                              tags$b("Time S5"),
                              tags$p(textOutput(outputId = "end_of_observation_name",container=div))),
                       column(8,
                              sliderInput(inputId = "y_sed_5",label='Value S5',value=0,min=-1,max=1,step=0.05,animate = TRUE))
                     ),
                     ...
                     wellPanel(
                               ...
                               numericInput(inputId = "max_slider_val_sed",
                                            label = 'Maximum Slider Value for Sedimentation Rate [m per Myr]',
                                            value=1,
                                            min=0,
                                            step=0.1),
                               numericInput(inputId = "max_slider_val_pat",
                                            label = 'Maximum Slider Value for Shell Input [# per Myr]',
                                            value=1,
                                            min=0,
                                            step=0.1),
                               actionButton('update_slider_options',"Apply Changes")
                     )
          )
      )
  ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$go_to_app,{ updateTabsetPanel(session, inputId = "condensation_app",
                                                   selected = "app")})
  observeEvent(input$go_to_app_2,{ updateTabsetPanel(session, inputId = "condensation_app",
                                                   selected = "app")})
  ...
  xdep=reactive({c(0,req(input$x_sed_2),req(input$x_sed_3),req(input$x_sed_4),duration())})
  xpat=reactive({c(0,req(input$x_pat_2),req(input$x_pat_3),req(input$x_pat_4),duration())})
  ydep=reactive({c(input$y_sed_1,input$y_sed_2,input$y_sed_3,input$y_sed_4,input$y_sed_5)})
  ypat=reactive({c(input$y_pat_1,input$y_pat_2,input$y_pat_3,input$y_pat_4,input$y_pat_5)})
  condensator_results=reactive({
    condensator(xdep=xdep(),
                ydep=ydep(),
                xpat=xpat(),
                ypat=ypat(),
                duration=duration(),
                ...
                plot_options = list(col_options=c(pattern_col='red',
                                    ...
                                    line_options=c(pattern_lwd=3,
                                                   ...
                                                   hiatus_lwd=5,
                                                   baseline_lwd=1),
                                    name_options=c(L_unit=height_unit(),
                                    ...
                                    par_options=c(no_of_ticks=5,
                                                  ...
                                                  label_expansion=1.3),
                                    points_plot=list(plot_sed_points=input$show_sed_points,
                                                     x_sed_points=xdep(),
                                                     y_sed_points=ydep(),
                                                     sed_points_pch=24,
